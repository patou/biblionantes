...
class SearchRepository {
  ...
  Future<List<Book>> search(String search,
    ...
    var list = response.data['resultSet']
  ...
  Future<BookDetail> detail(String id) async {
    ...
    if (response.statusCode != 200) {
      print("error");
      return Future.error(FetchDataException(
          'error occurred when get meta : {$response.statusCode}'));
    }
    Map<String, Map<String, dynamic>> summary =
        Map.fromIterable(response.data['summary'], key: (json) => json['name']);
    var creators = List.from(response.data['summary'])
        .where((item) => item['name'] == 'meta.creator')
        .toList();
    var book = Book(
        id: id,
        ...
        imageURL:
            'https://catalogue-bm.nantes.fr${summary['imageSource_128']?['value']}');
    var details = <Detail>[];
    if (creators.isNotEmpty) {
      details.addAll(creators
          .map((item) => Detail(
    ...
    if (summary['meta.publicationStatement'] != null) {
      details.add(Detail(
          display: summary['meta.publicationStatement']?['display'],
          value: summary['meta.publicationStatement']?['value'],
          icon: Icons.publish,
          order: 3));
    }
  ...
  Future<List<Detail>> info(String ark) async {
    final response =
        await client.get('resolveArk', queryParameters: {'ark': ark});
    if (response.statusCode != 200) {
      print("error");
      return Future.error(FetchDataException(
          'error occurred when get info on book: {$response.statusCode}'));
    }
    var details = <Detail>[];
    if (response.data['collectionData'] != null) {
      ...
      if (data['abstract'] != null) {
        details
            .add(Detail(display: "Résumé", value: data['abstract'], order: 0));
      }
      if (data['descriptionPhysical'] != null) {
        details.add(Detail(
            display: "Description",
            value: data['descriptionPhysical'],
            icon: Icons.format_size,
            order: 5));
      }
      if (data['noteTargetAudience'] != null) {
        details.add(Detail(
            display: "Public concerné",
            value: data['noteTargetAudience'],
      ...
      if (data['relationSet'] != null) {
        details.add(Detail(
            ...
            value: data['relationSet']['DocumentCaption'],
      ...
      if (data['subjectGeneral'] != null) {
        details.add(Detail(
            display: "Sujet",
            value: data['subjectGeneral']
                .map((item) => item['DocumentCaption'])
                .join(', '),
            icon: Icons.subject,
            order: 3));
      }
